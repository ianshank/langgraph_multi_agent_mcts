...
class TacticalAugmenter:
    ...
    def _augment_parameters(self, text: str, domain: Optional[str]) -> str:
        ...
        elif domain == "military" or any(
            kw in text.lower() for kw in ["tactical", "military", "reconnaissance"]
        ):
    ...
    def augment_batch(
        ...
        logger.info(
            f"Augmented {len(samples)} samples to {len(all_samples)} "
            f"(+{len(all_samples) - len(samples)} augmented)"
        )
...
class CyberSecurityAugmenter(TacticalAugmenter):
    ...
    def augment_with_mitre_context(self, sample: DatasetSample) -> DatasetSample:
        ...
        augmented_text = (
            f"[MITRE ATT&CK: {tactic}] [Severity: {severity}] " f"{sample.text}"
        )
...
class MilitaryTacticalAugmenter(TacticalAugmenter):
    ...
    def augment_with_force_composition(self, sample: DatasetSample) -> DatasetSample:
        ...
        augmented_text = (
            f"[Force: {force}] [Conditions: {condition}] " f"{sample.text}"
        )
