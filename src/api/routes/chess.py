...
_chess_adapter: ChessAdapter | None = None
...
async def search_best_move(request: ChessSearchRequest, adapter: ChessAdapter = Depends(get_chess_adapter)):
    try:
...
async def get_legal_moves(request: LegalMovesRequest, adapter: ChessAdapter = Depends(get_chess_adapter)):
    try:
...
async def report_game_feedback(feedback: GameFeedback, adapter: ChessAdapter = Depends(get_chess_adapter)):
    """
    ...
    logger.info("Received game feedback", extra={"moves": len(feedback.fen_history), "outcome": feedback.outcome})
