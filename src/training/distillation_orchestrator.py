...
from src.training.system_config import HRMConfig, SystemConfig, TRMConfig
...
class DistillationOrchestrator:
    ...
    def distill_hrm(
        ...
        dataset = DistillationDataset(data_dir=self.data_dir, task_type=DistillationTask.HRM_DECOMPOSITION)
        ...
        optimizer = torch.optim.AdamW(
            list(self.hrm_agent.parameters()) + list(self.decoder.parameters()), lr=learning_rate
        )
        ...
        def decoding_loss_wrapper(latent_state, target_text_list):
            ...
            target_tokens = self.decoder.tokenizer(
                target_text_list, padding=True, truncation=True, return_tensors="pt", max_length=128
            ).to(self.device)

            target_ids = target_tokens.input_ids

            # Forward pass decoder
            outputs = self.decoder(latent_state=latent_state, target_ids=target_ids)
            return outputs.loss
    ...
    def distill_trm(
        ...
        dataset = DistillationDataset(data_dir=self.data_dir, task_type=DistillationTask.TRM_REFINEMENT)
        ...
        optimizer = torch.optim.AdamW(
            list(self.trm_agent.parameters()) + list(self.decoder.parameters()), lr=learning_rate
        )
        ...
        for epoch in range(num_epochs):
            ...
            for batch in dataloader:
                ...
                dec_outputs = self.decoder(latent_state=output.final_prediction, target_ids=target_tokens.input_ids)
    ...
    def distill_policy_value(
        self, batch_size: int = 32, num_epochs: int = 10, learning_rate: float = 1e-4
    ) -> dict[str, float]:
        """
        ...
        dataset = DistillationDataset(data_dir=self.data_dir, task_type=DistillationTask.POLICY_VALUE)
    ...
    def distill_meta_controller(
        ...
        dataset = DistillationDataset(data_dir=self.data_dir, task_type=DistillationTask.META_CONTROLLER)
        ...
        for epoch in range(num_epochs):
            ...
            for batch in dataloader:
                ...
                inputs = self.meta_controller.tokenizer(
                    batch_text, padding=True, truncation=True, return_tensors="pt", max_length=128
                ).to(self.device)
