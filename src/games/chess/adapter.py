...
from src.games.chess.state import create_state_from_fen
...
class ChessAdapter:
    ...
    def __init__(self, config: SystemConfig, model_path: str | None = None):
        self._config = config
    ...
    def search(self, fen: str, simulations: int | None = None) -> dict[str, Any]:
        """
        ...
        mcts = NeuralMCTS(config=self._mcts_config, model=self._network, device=self._device)
