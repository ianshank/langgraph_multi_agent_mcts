...
from src.framework.mcts.llm_guided.config import LLMGuidedMCTSPreset, create_llm_mcts_preset
...
async def benchmark():
    ...
    orch_llm = UnifiedSearchOrchestrator(
        ...
        mcts_config,
        config_llm,
        hrm_adapter=hrm_adapter_llm,
        trm_adapter=trm_adapter_llm,
    ...
    config_neural = IntegrationConfig(use_hrm_decomposition=True, distillation_mode=False)

    hrm_adapter_neural = HRMAdapter(
        hrm_agent=hrm_agent, llm_client=llm_client, config=config_neural, encoder=encoder, decoder=decoder
    )
