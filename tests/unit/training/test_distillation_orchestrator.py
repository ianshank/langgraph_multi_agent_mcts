...
def orchestrator(mock_config, tmp_path):
    with (
        patch("src.training.distillation_orchestrator.SystemEncoder", return_value=MockEncoder()),
        patch("src.training.distillation_orchestrator.SystemDecoder") as MockDecoderParams,
    ):  # Use class patch
        ...
        orch = DistillationOrchestrator(config=mock_config, data_dir=tmp_path / "data", output_dir=tmp_path / "models")
        # Inject mocks again
...
def test_distill_hrm(orchestrator):
    ...
    # Mock dataset
    with (
        patch("src.training.distillation_orchestrator.DistillationDataset") as MockDataset,
        patch("torch.save") as mock_save,
    ):
        mock_ds = MagicMock()
        mock_ds.__len__.return_value = 10
        mock_ds.__getitem__.return_value = {"problem": "Write foo", "target_decomposition": ["Step 1", "Step 2"]}
