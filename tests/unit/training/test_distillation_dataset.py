...
def sample_data(tmp_path):
    ...
    metadata = EpisodeMetadata(
        episode_id="test_ep", problem_type="code_generation", difficulty="easy", start_time=1000.0, outcome=1.0
    )
...
def test_load_dataset(sample_data):
    ...
    dataset = DistillationDataset(data_dir=sample_data, task_type=DistillationTask.POLICY_VALUE)
...
def test_hrm_mapping(sample_data):
    ...
    dataset = DistillationDataset(data_dir=sample_data, task_type=DistillationTask.HRM_DECOMPOSITION)
...
def test_meta_controller_mapping(sample_data):
    ...
    dataset = DistillationDataset(data_dir=sample_data, task_type=DistillationTask.META_CONTROLLER)
