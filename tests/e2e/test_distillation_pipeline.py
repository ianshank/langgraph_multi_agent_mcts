...
from src.training.distillation_dataset import EpisodeMetadata, TrainingExample
...
def create_synthetic_data(data_dir: Path):
    ...
    examples.append(
        TrainingExample(
            ...
            q_value=0.0,
            action="refine",
            test_results={"refined_code": "def add(a, b): return a + b", "refinement_metadata": {"improvement": 1.0}},
        )
    )
...
def test_e2e_distillation_loop(e2e_env):
    ...
    with (
        patch("src.agents.common.system_encoder.AutoTokenizer") as MockEncTok,
        ...
        orchestrator = DistillationOrchestrator(config=config, data_dir=data_dir, output_dir=output_dir)
